...
def transcription():
    wmodel = whisper.load_model("large-v1")
    st.title("Speech to Text Transcriber")

    uploaded_file = st.file_uploader(label='Upload any audio file!')
    ...
    if st.session_state.file_loaded == False:
        if uploaded_file is not None:
            ...
            with st.spinner("Transcribing {} ...".format(file_name)):
                temp_dir = tempfile.TemporaryDirectory()
                temp_filepath = os.path.join(temp_dir.name,file_name)
                with open(temp_filepath,'wb') as f:
                    f.write(uploaded_file.getvalue())
                
                result = wmodel.transcribe(temp_filepath)

                
                st.success("File Transcribed Successfully!!")
                st.session_state.file_loaded = True

                st.subheader('Loaded Audio File', divider='rainbow')
                st.audio(temp_filepath,format="audio/{}".format(file_extension[1:]))

                st.subheader('Transcribed Text below', divider='rainbow')
                st.text(result["text"])
    else:
        if uploaded_file is None:
            st.session_state.file_loaded = False
...
def main():
    ...
    functions = ["Document Chatbot", "Transcription", ]
